...
impl Plugin for EmailPlugin {
	fn new() -> Self {
		...
		APP.options
			.arg(clap::Arg::new("email::from").long("email-from").takes_value(true));
		APP.options
			.arg(clap::Arg::new("email::reply-to").long("email-reply-to").takes_value(true));

		EmailPlugin { monitor: None }
	}
...
impl EmailPlugin {
	fn recv(mut monitor: Receiver, app: QuitHandle) {
		APP.spawn(async move {
			if let Ok(msg) = monitor.try_recv() {
				...
				let to = get_string(parsed_msg, "to").unwrap();
				let subject = get_string(parsed_msg, "subject").unwrap();
				let body = get_string(parsed_msg, "body").unwrap();

				if let Err(result) = Self::send(to.as_str(), subject.as_str(), body.as_str()) {
					log::error!("{}", result);
				}
			}
			if !app.is_quitting() {
	...
	pub fn send(to: &str, subject: &str, body: &str) -> Result<(), ExpectedError> {
		let smtp_username = libs::opt::get_value::<String>("email::smtp-username")?;
		let smtp_password = libs::opt::get_value::<String>("email::smtp-password")?;
		let credentials = Credentials::new(smtp_username, smtp_password);
		let smtp_relay = libs::opt::get_value::<String>("email::smtp-relay")?;
		let from = libs::opt::get_value::<String>("email::from")?;
		let reply_to = libs::opt::get_value::<String>("email::reply-to")?;
